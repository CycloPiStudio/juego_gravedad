...
func update_animation(velocity: Vector2) -> void:
	...
	if abs(velocity.x) > 10.0:
		...
		if aspirado:
			...
			if $AnimationPlayer.is_connected( "animation_finished", self, "aspirados")==false:
				# conecto el final de la animacion con la func aspirados
				conectaranima = $AnimationPlayer.connect("animation_finished",self,"aspirados")
#			
		else:
...
func moverse():
	...
	$AreaDanoEnemigo1/CollisionShape2D.set_disabled(false)
...
func aspirando():
	...
	$AreaDanoEnemigo1/CollisionShape2D.set_disabled(true)
	snap = false
	gravity = 0
#	
